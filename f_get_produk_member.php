...
$oe = mysqli_query($koneksi,"select * from user where username = '$_GET[email]'");
$oe2 = mysqli_fetch_assoc($oe);

if(empty($oe2['member_dari'])){
    $iddd = $oe2['idUser'];
}else{
    $iddd = $oe2['member_dari'];
}


$sql_get_data = "select * from tb_produk_member where id_member_vip = '$iddd' and visibility = '1' order by id_produk desc";
...

{
      ...
      "id_member": "<?php echo $row['id_produk'] ?>",
