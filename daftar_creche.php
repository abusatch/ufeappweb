 ...
 if(empty($_POST['username'])){
  echo "[";
    ...
    if(empty($_POST['username'])){
      ...
      $ncd = mysqli_query($koneksi,"select * from user where username = '$_POST[username2]'");
      $ncd2 = mysqli_fetch_assoc($ncd);
   ...
   if(empty($_POST['username'])){
    ...
    $query = mysqli_query($koneksi,"SELECT * FROM user where idUser = '$ncd2[idUser]' and password = '$pss'  ");
...
<?php
    ...
    <?php
      ...
      if(empty($_POST['username'])){
        ...
        mysqli_query($koneksi,"insert into tb_order (
          ...
          null,'3','creche','$ncd2[idUser]','$_POST[nama_produk]','$_POST[nama_depan]','$_POST[nama_belakang]','$_POST[tempat_lahir]'
  ...
  values
  ('Ordre créche','Merci d’avoir commander, s’il vous plaît effectuer le paiement','$ncd2[idUser]','-','-','$tanggal','$tanggal2')");

  
  $pl = mysqli_query($koneksi,"select * from tb_order where id_user = '$ncd2[idUser]' order by id_order desc");
...
$noin = "INV/".$ncd2['idUser'].$pl2['id_order'].$stop_date;
...
{
  "success": 1,
  "message": "ini ya - <?php echo $row2.' - '.$ncd2['idUser'] ?> - dan ini <?php echo $dibaca ?> ket <?php echo mysqli_error($koneksi); ?>",
  "id": "<?php echo $row2 ?>",
