...
if($_GET['filter'] == ""){
  if(!empty($_GET['id'])){
    ...
    $pd = mysqli_query($koneksi,"select * from tb_actualite where visibility = '1' order by tanggal desc limit 1");
    $pd2 = mysqli_fetch_assoc($pd); 

$sql_get_data = "select * from tb_actualite where visibility = '1' order by tanggal desc limit 1";
...
"berita<?php echo $nom ?>_url":"<?php echo $nh2['url'] ?>",
