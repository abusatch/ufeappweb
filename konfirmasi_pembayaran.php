...
if($_POST['id_user'] == "web"){
$wq = mysqli_query($koneksi,"select * from user where username = '$_POST[id_user2]'");
}else{
...
$wq3 = mysqli_fetch_array($wq);	
	...
	if ($wq2 == 0) { 
		$response = new emp();
		$response->success = 0;
		$response->message = "Akses ditolak -".$_COOKIE['email']."-"; 
...
$lu2 = mysqli_fetch_array($lu);
		...
		$ewww = mysqli_query($koneksi,"select * from user where idUser = '$lu2[idUser]'");
...
$callNoti->sendNoti($titleNoti, $bodyNoti,$gambarrr);
        ...
        if($_POST['jenis_order'] == "memberufe"){
           ...
           $tanggalinput2 = date('Y-m-d H:i:s');
           
           $oe = mysqli_query($koneksi,"select * from tb_order_aktif where id_user = '$khj2[id_user]' and id_order = '$_POST[id_order]'");
           $oe2 = mysqli_num_rows($oe);
           ...
           if($hj2['harga'] == "5000000"){
			...
			$ggh = mysqli_query($koneksi,"select  * from tb_order_aktif 
			where id_order = '$_POST[id_order]'
			");
			if(mysqli_num_rows($ggh) == 0){
